                ...
                {payload.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium text-foreground">
                            {entry.name === '消费额度' ? `$${entry.value.toFixed(2)}` : entry.value.toLocaleString()}
                        </span>
                    </div>
                ))}
            </div>
...
const TeamConsumptionChart = ({ data = [], isLoading = false, className = '' }) => {
    ...
    return (
        <Card className={cn('@container/card h-auto', className)}>
            <CardHeader>
                <CardTitle>团队消费趋势</CardTitle>
                <CardDescription>
                    <span className="hidden @[540px]/card:block">
                        最近 {timeRange === '7d' ? '7' : timeRange === '30d' ? '30' : '90'} 天的请求数和消费趋势
                    </span>
...
TeamConsumptionChart.propTypes = {
...
export default TeamConsumptionChart;
